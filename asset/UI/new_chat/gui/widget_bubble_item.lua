...
function M:setMsg(msgData)
  self.textMsg:setVisible(msgData.msgType == MsgType.Text or msgData.msgType == MsgType.Emoji or msgData.msgType == MsgType.ShortMsg)
  ...
  elseif msgData.msgType == MsgType.Emoji then
    ...
    if data then
      ...
      self:setTextContent(string.format("\227\128\144%s\227\128\145", msg))
    end
...
function M:setTextContent(msg)
  ...
  elseif textWidth < self.textMsgMinWidth then
    self.textMsg:setWidth({
      0,
      self.textMsgMinWidth
    })
    textWidth = self.textMsgMinWidth
  end
  local textHeight = self.textMsg:getHeight()[2]
  self.ImageBubbleBg:setWidth({
    0,
    textWidth + self.imageBubbleWidthGap
  })
  self.ImageBubbleBg:setHeight({
    0,
    textHeight + self.imageBubbleHeightGap
  })
  self:setHeight({
    0,
    self.ImageBubbleBg:getHeight()[2] + self.rootHeightGap
  })
end
...
function M:fixShowMsg(str, len)
  ...
  len = len or chatSetting.chatBubbleSetting.chatLimit
