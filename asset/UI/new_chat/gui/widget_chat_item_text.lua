...
M.VoiceImageList = {
  "set:chat.json image:img_0_voiceicon_other",
  "set:chat.json image:img_0_voiceicon_other01",
  "set:chat.json image:img_0_voiceicon_other02"
}
M.VoiceImageListSelf = {
  "set:chat.json image:img_0_voiceicon_self",
  "set:chat.json image:img_0_voiceicon_self01",
  "set:chat.json image:img_0_voiceicon_self02"
}
...
function M:initEvent()
  function self.imageBubbleVoice.onWindowClick()
    self:playSound()
  end
...
function M:setMsgContent(msgData)
  self.panelText:setVisible(msgData.msgType == Define.MsgType.Text or msgData.msgType == Define.MsgType.ShortMsg)
  self.imageEmoji:setVisible(msgData.msgType == Define.MsgType.Emoji)
  self.panelVoice:setVisible(msgData.msgType == Define.MsgType.Voice)
  self:setHeight({
    0,
    self.rootInitHeight
  })
  if msgData.msgType == Define.MsgType.Text or msgData.msgType == Define.MsgType.ShortMsg then
    self:setTextWidget(self.textMsg, self.textMsgInitW, self.textMsgInitH)
    local textSize = self.textMsg:getSize()
    local bubbleSize = {
      width = {
        0,
        textSize.width[2] + self.imageBubbleWidthGap
      },
      height = {
        0,
        textSize.height[2] + self.imageBubbleHeightGap
      }
    }
  ...
  elseif msgData.msgType == Define.MsgType.Emoji then
...
function M:setTextWidget(textWidget, initW, initH)
  ...
  textWidget:setProperty("HorzFormatting", "LeftAligned")
  textWidget:setText(msgText)
  if initW < textWidget:getWidth()[2] then
    textWidget:setProperty("AutoScale", "2")
    textWidget:setProperty("HorzFormatting", "WordWrapLeftAligned")
...
function M:setSide(msgData, isLeft)
  ...
  self.panelHead:setXPosition({
    0,
    self.panelHeadOffsetX * sign
  })
  self.textName:setHorizontalAlignment(alignmentH)
  self.textName:setXPosition({
    0,
    self.textNameOffsetX * sign
  })
  if msgData.msgType == Define.MsgType.Text or msgData.msgType == Define.MsgType.ShortMsg then
    self.textMsg:setHorizontalAlignment(alignmentH)
    self.textMsg:setXPosition({
      0,
      self.textMsgOffsetX * sign
    })
    self.panelText:setHorizontalAlignment(alignmentH)
    self.panelText:setXPosition({
      0,
      self.panelTextOffsetX * sign
    })
    if isLeft then
      self.imageBubble:setImage("set:chat.json image:img_9_bubble_other")
      self.imageBubble:setProperty("StaticImageStretch", "22 21 19 14")
    else
      self.imageBubble:setImage("set:chat.json image:img_9_bubble_self")
      self.imageBubble:setProperty("StaticImageStretch", "17 22 31 11")
    end
  elseif msgData.msgType == Define.MsgType.Emoji then
    self.imageEmoji:setHorizontalAlignment(alignmentH)
    self.imageEmoji:setXPosition({
      0,
      self.imageEmojiOffsetX * sign
    })
  elseif msgData.msgType == Define.MsgType.Voice then
    self.imageBubbleVoice:setHorizontalAlignment(alignmentH)
    self.panelVoice:setHorizontalAlignment(alignmentH)
    self.panelVoice:setXPosition({
      0,
      self.panelTextOffsetX * sign
    })
    if isLeft then
      self.imageBubbleVoice:setImage("set:chat.json image:img_9_bubble_other")
      self.imageBubbleVoice:setProperty("StaticImageStretch", "28 30 12 12")
    else
      self.imageBubbleVoice:setImage("set:chat.json image:img_9_bubble_self")
      self.imageBubbleVoice:setProperty("StaticImageStretch", "14 30 28 12")
    end
    if self:isSelfMsg() then
      self.imageVoiceSelf:setHorizontalAlignment(alignmentH)
      self.imageVoiceSelf:setXPosition({
        0,
        self.imageVoiceOffsetX * sign
      })
    else
      self.imageVoiceOther:setHorizontalAlignment(alignmentH)
      self.imageVoiceOther:setXPosition({
        0,
        self.imageVoiceOffsetX * sign
      })
    end
    self.textVoiceTime:setHorizontalAlignment(alignmentH)
    self.textVoiceTime:setXPosition({
      0,
      self.textVoiceTimeOffsetX * sign
    })
  end
end
