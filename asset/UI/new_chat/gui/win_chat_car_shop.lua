...
function M:initEvent()
  for _, v in pairs(self.cardItemList) do
    ...
    function item.panelBuy.onMouseClick()
  ...
  self._allEvent[#self._allEvent + 1] = Lib.subscribeEvent(Event.EVENT_CHAT_CARD_TIME, function(time)
    print(">>>>>>>>>>>>>>>>>>>> receive EVENT_CHAT_CARD_TIME:", time, math.floor(time / 86400))
    self.panelRemainDay:setVisible(-1 < time)
    if -1 < time then
      local day = math.floor(time / 86400)
      if 0 < day then
        self.textRemainDay:setText(Lang:toText({
          "new.chat.moonTime",
          day
        }))
      else
